...
namespace XMPS2000.Configuration
{
    ...
    public class PublishRequest : Form
    {
        XMPS xm;
        ...
        public int TopicId;
        public int PubKeyVal;
        private bool isEditOn = false;

        public PublishRequest(int topicId)
        {
            xm = XMPS.Instance;
            errorProvider = new ErrorProvider();
            InitializeComponent();
            this.textTopic.Text = xm.GetTopicName("Publish", topicId);
            TopicId = topicId;
            FillTags();
            var chkpublist = xm.LoadedProject.Devices.Where(d => d.GetType().Name == "Publish").Cast<Publish>().ToList();
            PubKeyVal = chkpublist.Where(s => s.keyvalue == topicId).Select(s => s.PubRequest.Count()).FirstOrDefault();
        }

        //Adding tagList to ComboBOxTagList
        ...
        public PublishRequest(PubRequest pubReq, string topicName = "", bool isEdit = false)
        {
            this.isEditOn = false;
            xm = XMPS.Instance;
            errorProvider = new ErrorProvider();
            InitializeComponent();
            FillTags();
            TopicId = pubReq.topic;
            PubKeyVal = pubReq.Keyvalue;
            this.textTopic.Text = topicName == "" ? xm.GetTopicName("Publish", pubReq.topic) : topicName;
            this.textKeyname.Text = pubReq.req;
            if (!pubReq.Tag.Contains(":"))
            {
                this.comboBoxTaglist.Text = pubReq.Tag;
                this.textNewtag.Text = XMProValidator.GetTheAddressFromTag(pubReq.Tag);
            }
            else
            {
                this.comboBoxTaglist.Text = XMProValidator.GetTheTagnameFromAddress(pubReq.Tag);
                this.textNewtag.Text = pubReq.Tag;
            }
            this.isEditOn = isEdit;
        }

        public PublishRequest(PubRequest pubReq, int topicId, string topicName = "")
        {
            this.isEditOn = false;
            xm = XMPS.Instance;
            errorProvider = new ErrorProvider();
            ...
            this.textTopic.Text = topicName == "" ? xm.GetTopicName("Publish", pubReq.topic) : topicName;
            this.textKeyname.Text = pubReq.req;
            this.comboBoxTaglist.SelectedIndex = pubReq.Tag.Contains(":") ? comboBoxTaglist.Items.IndexOf(XMProValidator.GetTheTagnameFromAddress(pubReq.Tag)) : comboBoxTaglist.Items.IndexOf(pubReq.Tag);
            this.textNewtag.Text = pubReq.Tag.Contains(":") ? pubReq.Tag : XMProValidator.GetTheAddressFromTag(pubReq.Tag);
            TopicId = topicId;
        }
        private void InitializeComponent()
        {
            ...
            this.lblKeyname.Location = new System.Drawing.Point(19, 42);
            ...
            this.lblVariable.Location = new System.Drawing.Point(19, 81);
            ...
            this.textTopic.Location = new System.Drawing.Point(93, 12);
            ...
            // textKeyname
            // 
            this.textKeyname.Location = new System.Drawing.Point(93, 39);
            ...
            this.comboBoxTaglist.Location = new System.Drawing.Point(93, 78);
            ...
            // textNewtag
            // 
            this.textNewtag.Location = new System.Drawing.Point(258, 78);
            ...
            this.lblString.Location = new System.Drawing.Point(258, 42);
            ...
            // btnSave
            // 
            this.btnSave.Location = new System.Drawing.Point(258, 119);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                var chkpublist = xm.LoadedProject.Devices.Where(d => d.GetType().Name == "Publish").Cast<Publish>().ToList();
                Publish subtop = chkpublist.Where(s => s.keyvalue == TopicId).FirstOrDefault();
                if (subtop != null)
                {
                    ...
                    if (subtop.PubRequest.Count() >= maxMQTTReqCount  && !isEditOn)
                    {
                    ...
                    if (subtop.PubRequest.Where(r => r.req == textKeyname.Text.ToString() && r.Keyvalue != PubKeyVal).Count() > 0 && isEditOn)
                    {
                        MessageBox.Show("Duplicate keyname found, Key name can't be added", "XMPS 2000", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    if (subtop.PubRequest.Where(r => r.req == textKeyname.Text.ToString() && r.topic == TopicId).Count() > 0 && !isEditOn)
                    {
                        MessageBox.Show("Duplicate keyname found, Key name can't be added", "XMPS 2000", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                ...
                else
                {
                    ...
                    if (textNewtag.Text != "")
                    {
                        if (MessageBox.Show("Variable Tag is not Added You Want to Add", "XMPS 2000", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == DialogResult.Yes)
                        {
                            ...
                            userControl.Text = "Add New Address Added in Logic";
                            ...
                            {
                                comboBoxTaglist.DataSource = null;
                                FillTags();
        ...
        {
            if (textKeyname.Text.Length > 50)
            {
                errorProvider.SetError(textKeyname, "The topic Length must be less than or equal to 24");
                e.Cancel = true;
            }
            else
            {
